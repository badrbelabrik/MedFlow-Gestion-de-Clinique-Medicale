...
?>
    ...
    <main class="flex-grow max-w-4xl w-full mx-auto px-4 sm:px-6 py-10 space-y-12">
        ...
        <section class="bg-white border border-slate-200/80 rounded-3xl p-6 sm:p-8 shadow-sm space-y-8">
            ...
            <form action="/patient/dashboard" method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4 bg-slate-50 p-4 rounded-2xl border border-slate-100">
                <div>
                    <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Nom du médecin</label>
                    <div class="relative">
                        <span class="absolute inset-y-0 left-0 flex items-center pl-3 text-slate-400 text-xs">🔍</span>
                        <input type="text" name="search_name" placeholder="Ex: Bennani..." class="w-full border border-slate-200 bg-white rounded-xl pl-8 pr-3 py-2.5 text-sm text-slate-800 focus:ring-2 focus:ring-emerald-500 transition">
                    </div>
                </div>
